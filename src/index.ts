import { Compositor } from "@cathodique/wayland-server-js-impl";
import { KeyboardRegistry } from "@cathodique/wayland-server-js-impl/dist/objects/wl_keyboard";
import { OutputRegistry } from "@cathodique/wayland-server-js-impl/dist/objects/wl_output.js";
import { SeatRegistry } from "@cathodique/wayland-server-js-impl/dist/objects/wl_seat.js";
import { interfaces } from "@cathodique/wayland-server-js-impl/dist/wayland_interpreter";
import { InstructionType, RegRectangle } from "@cathodique/wayland-server-js-impl/dist/objects/wl_region";
import { WlBuffer } from "@cathodique/wayland-server-js-impl/dist/objects/wl_buffer";
import { XdgWmBase } from "@cathodique/wayland-server-js-impl/dist/objects/xdg_wm_base";
...
compo.on('connection', (c) => {
  ...
  c.on('wl_surface', async (surf) => {
    console.log(surf);
    const awaitCommit = () => new Promise<WlBuffer>((r) => surf.once('commit', (v) => r(v)));
    ...
    if (!ctx) throw new Error('Failed to derive 2d context from canvas element; is anything disabled?');

    console.log(surf);

    let wasInSurface = false;

    function move(evt: MouseEvent, forceLeave?: boolean) {
      (surf.xdgSurface?.parent as XdgWmBase).addCommand('ping', { serial: surf.connection.time.getTime() });
    ...
    canvas.addEventListener('mousedown', (evt) => {
      ...
      if (wasInSurface) mySeatTransport.emit('buttonDown', webToButtonMap[evt.button]);
    });
    canvas.addEventListener('mouseup', (evt) => {
      console.log('up', webToButtonMap[evt.button]);
      if (wasInSurface) mySeatTransport.emit('buttonUp', webToButtonMap[evt.button]);
    });

    function commitHandler(b: WlBuffer | null) {
      console.log('hey', b);
      if (b == null) return;
