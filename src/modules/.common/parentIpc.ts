...
import { parentOrigin } from "./utils/utils.js";

// We are removing the * for now because
// we would rather trust the parent.
// TODO expose public handle with API.
const parentIpc = new OrderedPeer(window.parent, undefined, parentOrigin);
parentIpc.addHandler(new CathodiqueRemoteHandler());
parentIpc.addHandler(new CathodiqueProviderHandler(parentIpc));
